...
- **模型目录不必先保存档案才能读**：`model` 是必填项，而可选项此前只由“保存 → 能力探测 → 写能力缓存”这条链填充，于是新建本地/OpenAI 兼容档案时用户卡在一个必须先填、却只能靠上游才知道答案的空框上（探测按钮对未保存档案根本不出现）。模型行新增 **“拉取模型”**：`POST /ai_prompt_studio/models` 按**表单当前**的 API URL 与密钥只做一次 `GET {base}/models`——不写档案、不写能力缓存、响应不含密钥，密钥取“表单里刚输入的优先，其次用已保存档案的”，所以换 key 不必先保存就能看到能看到哪些模型；原地址拿不到目录时实测 `/v1`（LM Studio 的管理 API 在根路径、推理 API 在 `/v1`），命中就在提示里点名实际端点。结果同时填充“模型”与“视觉模型”候选（视觉 URL 与主 URL 不同时单独列一次）；恰好一个模型且模型框为空时直接填入。空模型现在也在提交前给出可操作提示（“点「拉取模型」从上游目录选择”），而不是等后端返回 `AIProfile: model 不能为空`。
...
- **删掉 ADR 0008 早已宣布移除的 H3 结构化计划孤立助手**：`services/h3_plan.py`（`H3_SCHEMA`、`build_plan_task_data`、`parse_plan_json`、`map_image_assets`、`sync_manifest_assets`、`normalize_media_labels`、`normalize_ref2va_summary`）与 `services/h3_studio_runtime.py` 里的 `normalize_plan`、`_inject_locked_identity`、`render_validate`、`MODE_IMAGE_REQUIREMENTS` 一并删除——生产节点只导入 `prepare_manifest`，这些函数只有 `tests/` 碰得到，等于把一条不通电的通道留在仓库里让文档为它辩护。参考图数量规则自此只有 `nodes/h3_prompt_studio._validate_lenient_h3` 一个归属者（此前节点与 `MODE_IMAGE_REQUIREMENTS` 各存一份字面量，必然漂移）。只覆盖死通道的测试随代码删除，其中唯一仍活在生产里的断言（H3 请求带类型化任务数据但不带抄写的 JSON Schema、Ref2VA 英文检查）改写为走节点/校验器的真实路径。`renderers/minimax_h3.py` 保留为输出格式的可执行规格，仍由 `tests/test_renderers_h3.py` 对着活的校验器验证。同时更正 `docs/decisions.md`（D15/D18/D24）、`docs/prompt-audit.md`、`docs/final-report.md` 与 `docs/prompt-architecture/`：真正经 `schema_contract` 接通的是 `STORYBOARD_SCHEMA`，`H3_SCHEMA` 从未接入生产。
...
- **附件按实际选中的协议复核**：带图片/文件附件而两种协议都没通过同格式探针时，`_select_protocol` 不再按文本能力回落到 Responses（工具仍是软要求，没有探针通过时照常退化为不带工具的回答），并在选定协议后用 `vision_chat`/`vision_responses`、`files_chat`/`files_responses` 复核一次。只有聚合 `vision`/`files` 为真不再能放行附件，错误信息点名是哪个协议没通过哪项探针。顺带去掉 `generate()` 里重复的一次能力读取。
...
- **设置面板不再被过期响应盖掉**：档案列表缓存增加失效代号，写操作在途时返回的旧响应不再回写缓存（此前新建/删除档案后列表会最长 15 秒显示旧内容，且切走再切回仍是旧的）；档案区、编辑区、能力区各自加“只有最后一次请求生效”的守卫，快速连点两个档案不再出现列表高亮 B、表单显示 A；点击一次档案行由 3 个重复请求降为 1 个（能力区复用已取回的档案数据）；“尚未实测”的提示改为指向真实存在的“能力探测”按钮。

- **DeepSeek 能力表与审计文档对齐**：明确 `DEEPSEEK_MODEL_CAPS` 只回答「未探测时敢不敢发」（当前 flash 与 pro 都标 `responses: False`，未探测一律走 Chat Completions），Responses 及其原生 `web_search`、`text.format` json_schema 只在主动探测实测通过后启用。修正网关里仍写着「flash→responses」的过期注释，新增测试保证能力表一旦标注可用就会真的选择 Responses（避免该分支成为死代码），并把 `docs/research.md`、`docs/known-limitations.md` 中混为一谈的「官方结论」与「实现开关」分开表述。

- **档案保存 / 密钥 / 能力探测改为单一权威**：保存档案不再无条件清空能力缓存（失效只按能力指纹判定，改名称、温度、超时等无关字段后探测结果与模型列表继续可用）；删除独立的“保存密钥”按钮，填写的密钥随“保存”一次写入，新建档案不再丢弃已填密钥；能力探测前若表单存在未保存修改会先提醒，并说明探测会按实测覆盖 API URL 与图片/文件支持勾选；两个支持勾选的提示改为与实现一致（实测优先）；清除密钥的提示改为“密钥已清除”。

- **移除重构约束**：删除 `AGENTS.md` 与 `docs/重构约束/` 两份架构契约及其状态跟踪文件（对应阶段的验收已完成，继续占用每次 UI/提示词改动的前置阅读），并移除锁定它们存在的加载器测试。

- **设置工作台改为纯中文**：删除前端双语字典与语言切换（面板内 EN 按钮、ComfyUI 原生「界面语言」设置项），文案直接写在控件上。同时补齐此前显示成原始字段名的标签（视觉档案、温度、Top P、频率惩罚、存在惩罚、最大输出 tokens、外部搜索地址、支持图片输入、支持文件输入、后端，以及 Markdown 资料编辑器的全部字段）；资料「标题」字段曾因 key 与面板标题撞名而显示成“AI Prompt Studio 设置”，现已修正。删除档案的提示由“已保存”改为“已删除”，网络失败提示带上真实失败原因。新增回归测试保证不再出现未翻译的 snake_case 标签。
