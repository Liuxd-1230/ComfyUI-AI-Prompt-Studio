...
def test_lenient_create_and_refine_commit_freeform_revisions(
    ...
    assert positive2.startswith("masterpiece, best quality, score_7, A woman in a red coat")
    ...
    assert "masterpiece, best quality, score_7, A woman walks through blue-hour rain." in sent
...
def test_public_node_persists_journal_and_forks_copied_session(
    ...
    node = studio_mod.APS_PromptStudio()
    original = node.run(
        _profile(store), "woman in rain", "anima_base",
    ...
    copied = node.run(
        ...
        "anima_base", prompt_session=original["result"][2],
...
def test_lenient_untagged_prompt_commits_with_warning(monkeypatch, store) -> None:
    ...
    result = studio_mod.APS_PromptStudio().run(
        ...
        target="anima_base")
    assert result["result"][0].startswith("masterpiece, best quality, score_7")
...
def test_lenient_protocol_garbage_repairs_once_without_committing_first_result(
    ...
    assert result["result"][0] == (
        "masterpiece, best quality, score_7, "
        "A complete English prompt after format repair.")
    assert session.revision == 1
...
def test_previous_restores_prompt_without_gateway_call(monkeypatch, store) -> None:
    ...
    restored = node.run(
        ...
        target="anima_base", session_action="previous",
    ...
    assert restored["result"][0] == (
        "masterpiece, best quality, score_7, First complete English prompt.")
