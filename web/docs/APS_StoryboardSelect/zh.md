...
输出 `STORY_ITEM` 是单项，接 Image Prompt Studio；`STORY_ITEM_LIST` 是结构化容器；`scene_text` 供预览；`character_ids` 是人物 ID；`batch_count` 是数量；`STORY_ITEMS` 是 ComfyUI 真列表输出，供支持列表输入的节点逐项执行。
