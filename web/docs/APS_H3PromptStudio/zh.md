...
- “高级设置 · Prompt Supplements”：默认收起；可不使用、自动加载 H3 适用资料，或显式多选。工作流只在隐藏兼容字段 `prompt_supplements` 保存稳定 ID，资料不能覆盖 H3 协议或校验器。
...
成功执行后，工作台会写回 Session 并把工作流标为未保存；请及时保存。若异常退出导致
后端成功版本比工作流新，重新打开时会明确询问是否恢复。复制节点会保留当前 H3 成品
和 revision，但建立新的会话 lineage，后续修改不会与原节点互相覆盖。
...
节点要求 `<PROMPT>完整官方 H3 文本</PROMPT>` 与 `<SUMMARY>摘要</SUMMARY>`。
无标签但完整的普通 H3 文本会带黄色警告接收；半截 JSON、标签或协议说明只保真修复
一次。节点不会在 validator 失败后暗中改剧情；修复后仍不合格就保留上一版。
