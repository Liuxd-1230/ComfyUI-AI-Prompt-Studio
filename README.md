...
- **设置工作台**：ComfyUI 内嵌面板，按模型档案、能力与连接、Markdown 资料与日志分区懒加载；支持 Esc 关闭和键盘焦点循环。本地模型的加载/卸载不在面板里做——用 `APS_RuntimeControl` 与 `APS_UnloadModel` 节点，或让你的本地服务自己跑 OpenAI 兼容 API 后按档案接入。
